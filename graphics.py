...
class GraphWin(tk.Canvas):
    ...
    def motion(self,e):
        self.moveX, self.moveY = e.x, e.y

    def checkMovement(self):
        self.update()      # flush any prior clicks
        if self.moveX == None or self.moveY == None:
            return
        x,y = self.toWorld(self.moveX, self.moveY)
        self.moveX = None
        self.moveY = None
        return Point(x,y)
    ...
    def getRightMouse(self):
        self.update()
        if self.mouseRX != None and self.mouseRY != None:
            x,y = self.toWorld(self.mouseRX, self.mouseRY)
            self.mouseRX = None
            self.mouseRY = None
            return Point(x,y)
        else:
